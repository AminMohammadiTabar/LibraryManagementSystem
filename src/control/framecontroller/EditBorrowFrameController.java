...
public class EditBorrowFrameController implements Initializable {
    @FXML
    Scene scene;
    @FXML
    private Stage stage;
    ...
    @FXML
    TextField personIdTxt,bookIdTxt,idTxt;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        editBtn.setOnAction(event -> {
            if (idTxt.getText().isEmpty()&& bookIdTxt.getText().isEmpty() && personIdTxt.getText().isEmpty() &&
                    borrowDatePicker.getValue().equals(null)) {
                Alert alert = new Alert(Alert.AlertType.ERROR, "Check The Fields!");
                alert.show();
            } else {
                if (!PersonController.findById(Integer.parseInt(personIdTxt.getText())).getName().isEmpty()
                        && !BookController.findById(Integer.parseInt(bookIdTxt.getText())).getName().isEmpty()
                        && !borrowDatePicker.getValue().equals(null)
                        && !BorrowController.findById(Integer.parseInt(idTxt.getText())).getName().isEmpty()) {
                    String message = "Are sure to apply changes ?" ;
                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION, message);
                    BorrowController.edit(Integer.parseInt(idTxt.getText()),Integer.parseInt(personIdTxt.getText()),
                            Integer.parseInt(bookIdTxt.getText()), borrowDatePicker.getValue());
                    clearText();
                } else {
                    Alert alert = new Alert(Alert.AlertType.ERROR, "Enter valid IDs!");
                    alert.show();
                }
            }
        });
